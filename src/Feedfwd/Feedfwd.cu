...
std::unique_ptr<float[],deleterRR_struct> LogisticReg::getzl(const int l) {
	const int Lp1 = sizeDimsvec.size(); // L = total number of Axons and so L+1 is total number of layers
	assert (l < Lp1);	// sanity check that l=1,2,...L
	int idx_axon = l-1; // ind_axon=0,1,...L-1, 0-based counting
	auto ptr = std::move( Axons[idx_axon].getzl() );

	return ptr;
}		

std::unique_ptr<float[],deleterRR_struct> LogisticReg::getDpsil(const int l) {
	const int Lp1 = sizeDimsvec.size(); // L = total number of Axons and so L+1 is total number of layers
	assert (l < Lp1);	// sanity check that l=1,2,...L
	int idx_axon = l-1; // ind_axon=0,1,...L-1, 0-based counting
	auto ptr = std::move( Axons[idx_axon].getDpsil() );
...
void LogisticReg::grad_desc_step(const float alpha_rate, int Mx) {
	const int Lp1 = sizeDimsvec.size(); // L = total number of Axons and so L+1 is total number of layers
	const int K = sizeDimsvec[Lp1-1]; // size dim. of the a^L output layer for axon L, i.e. \widehat{h}, the prediction
	const int SIZE_Y= K * m; 
	...
	auto ThetaL = std::move( Axons[Lp1-2].getTheta() );
	auto bL = std::move( Axons[Lp1-2].getb() );
	auto Dpsil = std::move( Axons[Lp1-2].getDpsil() ); 
	...
	int Nx = (SIZE_Y + Mx -1)/Mx;
		...
		// Calculate \Delta =\Delta_i^{\  \  k}=(\widehat{y}^k_{(i)}-y_{(i)}^k)/(\widehat{y}^k_{(i)} (1-\widehat{y}_{(i)}^k))
	// Delta = Delta_i^k \in \text{Mat}_{\mathbb{R}}(m,K)
	...
	Deltaxent_kernel<<<Nx,Mx>>>(SIZE_Y, y.get(), yhat.get(), Delta.get() );
	
	HadamardMultiply_kernel<<<Nx,Mx>>>(SIZE_Y, Dpsil.get(), Delta.get());  
	// Delta is now  ( \Delta \odot d\psi^{(L)}/ dz^{(L)} ) \in \text{Mat}_{\mathbb{R}}(m,K)
	...
	/* ===== grid, thread block size dimensions ===== */
		// M_x = number of threads in a (single) block in x-direction
	Nx = (SIZE_ONES + Mx -1)/Mx;
	setconstval_kernel<<<Nx,Mx>>>(m,1.0f, ones.get() );
 
	// this is a clever way to do summation over the \sum_{i=1}^m
	cublasSgemm(*handle_u.get(), 
		CUBLAS_OP_N, CUBLAS_OP_N, 1, K, m,  
	...
	//sanity check
/*	Axons[Lp1-2].move2Theta_from_ptr( dTheta);
/*	Axons[Lp1-2].move2b_from_ptr( dB);
*/
}
 ...
 * 				a numerical trick for the usual (mathematical) Kronecker delta function	 
 * */
void LogisticReg::grad_desc(  const int iterations, const float alpha_rate, int Mx)
{
	for (int iter=0; iter < iterations; iter++) 
	{
		feedfwd(Mx);
